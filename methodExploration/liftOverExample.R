...
tDF <- as.data.frame(reduce(tList))
...
# here we can isolate lonly regions

plot(a$distToNext, pch = 16, cex = .1)

head(a[a$distToNext > 50000 & a$distFromLast > 50000,])
...
group_by(a, group_name, seqnames, distToNext < 50000)





a <- summarise(group_by(tDF, group_name, seqnames), width = sum(width), start = min(start), end = max(end)) %>% 
...
hist((a$width), breaks = 1000, xlim = c(0,100000))


nrow(a)
nrow(a[a$width/5000 > .1,])

a2 <- a[a$width/a$queWidth > .1 & a$width/5000 > .1,]



hist((a2$width), breaks = 100)
plot(a2$queWidth, pch = 16, cex = .1)
